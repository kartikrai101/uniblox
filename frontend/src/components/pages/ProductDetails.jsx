        ...
        <div>
            ...
            <div className='flex items-start w-full px-[40px] py-[30px] space-x-10'>
                <div className='w-[35%] rounded-[10px] border-grey border-[1px] p-[20px]'>
                    <img src={product.imgUrl} className='' alt="item-image" />
                ...
                <div className='w-[65%] '>
                    <div className='rounded-[10px] border-grey border-[1px] p-[15px]'>
                        <p className='text-[20px] text-[#6c757d]'>{product.name}</p>
                        <p className='text-[22px] font-medium'>{product.description}</p>
                        <div className='flex items-center space-x-4 mt-[10px]'>
                            <p className='bg-[#bc6c25] text-white font-medium px-[10px] rounded-[5px]'>{product.rating}</p>
                            <p className='text-[18px]'><span className='font-bold'>{product.reviews}</span> Ratings & <span className='font-bold'>{product.prevMonth}</span> users bought the product last month</p>
                        </div>
                        <p className='text-[#007200] font-medium mt-[10px]'>Special Price</p>
                        <p className='text-[32px] font-medium'>₹{product.price}</p>
                        <p className='mt-[10px] text-[18px] font-medium'>Product Specifications</p>
                        <hr/>
                        <div className='mt-[10px]'>
                            {
                                product.specs.map((obj, key) => {
                                    return <p><span className='font-medium'>{obj.key}:</span> {obj.value}</p>
                                })
                            }
                        </div>
                    </div>
